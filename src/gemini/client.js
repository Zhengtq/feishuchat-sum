...
⚠️【关键排版要求 - 必须严格遵守】⚠️
飞书卡片的 Markdown 支持存在兼容性问题，你在输出时必须：
1. **绝对禁止使用 "#" 作为标题**（如 ### 核心要点），飞书不支持渲染此类标题。
2. **绝对禁止使用斜体**（如 *文本* 或 _文本_），这会导致排版错乱。
3. 请统一使用 **加粗**（如 **📌 核心要点**）来代替标题层级。
4. 正文尽可能使用无序列表 "-" 或有序列表 "1." 进行条理化展示。
...
语言层级清晰、简洁有力，不要逐条翻译聊天记录。如果内容较少简短说明即可。
